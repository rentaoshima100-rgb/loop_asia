...
// 4法人の役割（確定）。資本・親子関係・各社の住所などの詳細は別途確認。
// 注：登録支援機関＝株式会社ループ管財／認定日本語教育機関＝日本語学校（以前は役割が逆になっていた）
const GROUP = [
  { role: "組合本体",  name: "亜細亜交流事業協同組合",       desc: "監理支援機関。育成就労制度における受入監理・支援を担う中核です。" },
  { role: "グループ会社", name: "株式会社ループ",            desc: "建築・建設業。" },
  { role: "グループ会社", name: "株式会社ループ管財",        desc: "登録支援機関。特定技能外国人の法定10項目支援を担当します。" },
...
/* 組合によるサポート（Support）— 6項目。番号・タイトルは将来変更の可能性あり。
   ...
   送出国リストは公式9カ国に統一。ネパールの扱いは要確認）。 */
const SUPPORT = [
  { num: "01", title: "豊富な実績と信頼", icon: "shield",
    body: "設立以来、多数の受入企業様・外国人材（旧制度の技能実習生を含む）をサポートしてきた実績があります。蓄積されたノウハウと信頼ネットワークにより、スムーズな人材受入れを実現します。" },
  ...
  { num: "06", title: "多様な送り出し国とのネットワーク", icon: "globe",
    /* 送出国リストはサイト公式9カ国に統一。ネパール追加は要確認 */
    body: "ベトナム・カンボジア・タイ・インドネシア・中国・モンゴル・バングラデシュ・スリランカ・ラオスなど複数の送り出し国と連携。企業様のニーズに合った人材を安定的に供給できるネットワークを有しています。" },
];
...
const FAQ_DATA = [
  ...
  { cat: "受入後・サポート",
    ...
    /* 要確認: 依頼書原文の末尾「グループ内企業では不動産事業も行っております」は、グループ4社に不動産業が無いため削除。実在が確認できれば復活。 */
  { cat: "受入後・サポート",
